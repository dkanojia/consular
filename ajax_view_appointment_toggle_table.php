		...
		$id = $_POST['id'];
			...
			//SELECT `intId`, `status`, `username`, `mobile`, `imei`, `email`, `passport_no`, `booked_date`, `city`, `booked_city`, `created_on` FROM `appointments` WHERE 1
				...
				while($roFetApp = mysqli_fetch_assoc($rFetApp)){
						...
						// <td>'.$roFetApp['mobile'].'</td>
						// <td>'.$roFetApp['passport_no'].'</td>
						// <td>'.$status.'</td>
						// <td>'.$roFetApp['booked_date'].'</td>
						// <td>'.$roFetApp['city'].'</td>
						// <td>'.$roFetApp['created_on'].'</td>
						// <td><button class= "btn btn-danger btn-xs" id = "'.$roFetApp['intId'].'" onclick = delete_holiday(this.id)>CANCEL</button></td></tr>';
						$ii++;
				}
				echo $str;
